	...
	while($row = mysqli_fetch_array($res))
	{
		...
		if ($email_flag==1)
		{
			...
			if (count($emailArray) > 0) {
				...
				$headers .= 'From: <noreply@triteqlock.com>' . "\r\n";
				$headers .= 'Cc: ' . "\r\n";
				
				foreach ($message as $key => $value) {
					$body = $body . $value . "<br/><br/>";
				}
				$body = $body . "This is an automated Alert message from Triteq FreshTraq System. Please visit FreshTraq at www.freshtraq.com to manage your system and see more information.";  
				$body = $body . "<br/><br/>Triteq Lock & Security<br/>(847) 640-7002";			
				
				SendEmail($to, $subject , $body);
				
				$currtime = $currenttime->getTimestamp(); 
				foreach ($did as $key => $value) {
					$sql = "update device set device_lastnotify_time=" . $currtime . " where device_id=" . $value;
					mysqli_query($conn,$sql); 
				}
				
				echo "Send Notification : " . $loc_name . " , Emails : " . implode(",",$emailArray);
				
			} else {
				
				echo "Send Notification : " . $loc_name . " , NO Email address set";
