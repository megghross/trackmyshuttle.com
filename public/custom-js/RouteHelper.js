...
class RouteHelper {
    constructor(Route) {
        ...
        this.locationUpdateMessage = 'Location is being updated';
    ...
    RequestToServerForDetail(marker){
        ...
        $.ajax({
           url: $("body").data("offroute"),
            "method": "POST",
            data:{
                "serialNumber":marker.customInfo.serialNumber,
    ...
    UpdateLocation(marker){
        ...
        if(seconds>6){
            ...
            // if(nearestRoutePoint.distance>40){
                this.time = new Date();
                this.RequestToServerForLocationUpdate(marker);
                return this.locationUpdateMessage;
            // }
            // return "Location is being updated";
        }
        return this.locationUpdateMessage;
    }
    ...
    RequestToServerForLocationUpdate(marker){
        let msg = 'Off Route';
        let mapHelper = this;
        $.ajax({
            url: $("body").data("locationupdate"),
            "method": "POST",
            data:{
                "serialNumber":marker.customInfo.serialNumber,
                "lat" : marker.position.lat(),
            ...
            success: function(data){
                // data = JSON.parse(data);
                let now = new Date();
                mapHelper.locationUpdateMessage = "Message:"+ data.msg ;
            }
