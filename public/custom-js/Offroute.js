...
$(document).ready(function () {
    ...
    $(document).on('click', '.route-box', function (event) {
        ...
        selectedShuttleNumber = id+""+shuttleNumber;
            ...
            for (k = 0; k < shuttleData.devices.length; k++) {
                marker[id][k] = new google.maps.Marker({
                    ...
                    customInfo: {
                        shuttleNumber: id+""+k,
                        routeId: id,
                        serialNo: shuttleData.devices[k].serialNumber,
                        type: "Shuttle"
                    }
                });
                marker[id][k].setPosition(mPosition);
                if(shuttleData.devices[k].shuttleName=="Shuttle 3-OffRoute"){
...
function etaUpdatev2(coords, targetIndex, marker, id, km_h) {
    ...
    try {
        ...
        if (shuttleNumber == selectedShuttleNumber) {
            ...
            let res = mapHelper[id].DetectOffRoute(marker);
            showETA(res);
            // document.getElementById("etaMessage").innerHTML = res;
            return true;
...
function showETA(EtaMessage) {
    $(".eta-details").show();
    $(".eta-details").html("");
    htmlStr = '<button type="button" onclick="hideEta()" class="hideKeyBox">X</button><table><caption>Off Route Detection</caption' +
